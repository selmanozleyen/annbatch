...
pure-Python path when the extension is not compiled.
...
# ---------------------------------------------------------------------------
# pread cache -- keeps open fds and pre-parsed shard indices
# ---------------------------------------------------------------------------
class _PreadCache:
    """Cache of open file descriptors + parsed shard indices (no mmap)."""

    __slots__ = ("_cache",)

    def __init__(self) -> None:
        self._cache: dict[str, tuple[int, np.ndarray, int]] = {}

    def get(
        self, shard_path: Path, shard_index_size: int, chunks_per_shard: tuple[int, ...]
    ) -> tuple[int, np.ndarray, int]:
        """Return (fd, shard_index, file_size) for the given shard file."""
        key = str(shard_path)
        if key not in self._cache:
            fd = os.open(key, os.O_RDONLY)
            file_size = os.fstat(fd).st_size
            idx_offset = file_size - shard_index_size
            idx_bytes = os.pread(fd, shard_index_size - 4, idx_offset)
            shard_index = np.frombuffer(idx_bytes, dtype="<u8").reshape(chunks_per_shard + (2,))
            self._cache[key] = (fd, shard_index, file_size)
        return self._cache[key]

    def clear(self) -> None:
        for fd, _, _ in self._cache.values():
            try:
                os.close(fd)
            except OSError:
                pass
...
def read_pread_dense(arr: zarr.Array, boundaries: np.ndarray) -> np.ndarray:
    """Like read_direct_dense but uses pread(2) instead of mmap."""
    (
        disk_root,
        shape,
        shard_shape,
        ...
        for i in range(len(starts)):
            ...
            while row < sel_stop:
                ...
                if _HAS_C_EXT:
                    ...
                    row, out_offset = _pread_dense_python_fallback(
                        fd,
                        idx_flat,
                        out,
                        blosc_codec,
...
def _pread_dense_python_fallback(
    fd,
    idx_flat,
    out,
    blosc_codec,
    ...
    """Pure-Python pread fallback for dense shard reading."""
    while row < sel_stop and row < shard_base + shard_row_size:
        inner_idx = (row - shard_base) // inner_row_size
        inner_base = shard_base + inner_idx * inner_row_size
        inner_end = inner_base + inner_row_size
        ...
        chunk_bytes = os.pread(fd, length_val, offset_val)
        decoded = blosc_codec.decode(chunk_bytes)
        chunk_arr = np.frombuffer(decoded, dtype=dtype).reshape(inner_chunk_shape)
...
def read_pread_1d(arr: zarr.Array, boundaries: np.ndarray) -> np.ndarray:
    """Like read_direct_1d but uses pread(2) instead of mmap."""
    if isinstance(arr, zarr.AsyncArray):
        arr = zarr.Array(arr)
...
def _pread_1d_python_fallback(
    ...
    """Pure-Python pread fallback for 1-D shard reading."""
    while pos < sel_stop and pos < shard_base + shard_size:
        inner_idx = (pos - shard_base) // inner_size
        inner_base = shard_base + inner_idx * inner_size
        inner_end = inner_base + inner_size
...
def get_read_dense(backend: IoBackend | None = "pread"):
    """Return the dense reader function for the given I/O backend, or None."""
    if backend is None:
        return None
    try:
...
def get_read_1d(backend: IoBackend | None = "pread"):
    """Return the 1-D reader function for the given I/O backend, or None."""
    if backend is None:
        return None
    try:
        return _1D_DISPATCH[backend]
    except KeyError:
        raise ValueError(f"Unknown io_backend: {backend!r}. Choose from {list(_1D_DISPATCH)}") from None
