...
class CategorySampler(Sampler[list[slice]]):
    ...
    __slots__ = ("_batch_size", "_shuffle_categories", "_shuffle_within", "_unique_categories", "_category_data", "_n_iters", "_is_sorted")
    ...
    def __init__(
        ...
        *,
        ...
        if self._is_sorted:
            ...
            self._category_data = {cat: (int(start), int(start + count)) for cat, start, count in zip(self._unique_categories, first_idx, counts, strict=True)}
        else:
    ...
    def __iter__(self) -> Iterator[list[slice]]:
        ...
        for cat in categories:
            ...
            if self._is_sorted:
                start, stop = data
                yield from RangeSampler(start=start, stop=stop, batch_size=self._batch_size, shuffle=self._shuffle_within)
            else:
